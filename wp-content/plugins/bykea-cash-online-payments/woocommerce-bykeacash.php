...
class OnlinePayments_BykeaCash extends WC_Payment_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		if($response->subcode=='10001'){
			...
			$orderNote = __("Bykea Cash invoice is successfully created. Invoice number is <a href='https://invoice.bykea.cash/receive/invoice/".$response->invoice_no."' target='_blank'>".$response->invoice_no."</a>");
			$order->add_order_note( $orderNote );
			// Redirect to Bykea Cash.
			if($this->payment_option=="card_payments"){
				$invoiceURL = 'https://invoice.bykea.cash/receive/card/'.$response->invoice_no;
