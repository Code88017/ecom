	...
	class WC_HblPay_Gateway extends WC_Payment_Gateway
	{
		...
		public function init_form_fields()
		{
				...
				'description' => array(
					...
					'default'     => 'Pay through HBL payment gateway. and whatever you want',
				),
				'client_name' => array(
					'title'       => 'Client Name',
					'type'        => 'text'
...
//show cards if available and select from them-----------MZ
	 ...
	 function validate_fields()
		...
		{
			error_log('::::::::::::::::process_payment STARTED::::::::::::::::::');
			global $woocommerce;
			$order = wc_get_order($order_id);
			...
			else
			{
			 ...
			 $f_response = callAPI('POST', $api_url, $jsondata);
			 //error_log("afterapi call...".json_decode($f_response,true));
			// $jsonData = json_decode(wp_remote_retrieve_body($f_response), true);
			// $jsonData = json_decode($f_response,true);

			// error_log("jsondata..".json_encode($f_response, JSON_PRETTY_PRINT));
			 //die();

			 $f_url = $page_url . encodeString($f_response['Data']['SESSION_ID']);
			// error_log(print_r($jsonData['Data']['SESSION_ID'].$token_valid,1));

			if(!isset($f_response['Data']['SESSION_ID']))
			{
				return false;
			}

			error_log('::::::::::::::::process_payment ENDED WITH SUCCESS URL'. $f_url .'::::::::::::::::::');
			return array(
				'result'   => 'success',
				'redirect' => $f_url
			);
		...
		{
			error_log('::::::::::::::::getRequestObject STARTED::::::::::::::::::');
			global $post;
...
// );
	...
	$custom_data = array (
			...
			error_log('::::::::::::::::getRequestObject ENDED::::::::::::::::::'.json_encode($custom_data,JSON_PRETTY_PRINT));
			return $custom_data;
		}
 	}
	...
	function callAPI($method, $url, $data)
	{
	 try
	 {
	 	error_log('::::::::::::::::callAPI Started::::::::::::::::::');
		// 	//error_log("api data....". json_encode($data, JSON_PRETTY_PRINT));
		 	$admin_settings = WC()->payment_gateways->payment_gateways()['hblpay']->settings;
...
if($admin_settings['use_proxy'] === 'yes')
 {
	 		$proxy = $admin_settings['proxy'];
			$proxy_settings=explode(':', $proxy);
			//error_log(print_r($proxy_settings,1));
			/* Configure HTTP Proxy Server */
			define('WP_PROXY_HOST', $proxy_settings[0].':'.$proxy_settings[1]);
			define('WP_PROXY_PORT', $proxy_settings[2]);
...
// Set the content type to application/json and add a body
 ...
 //error_log(print_r($response,1).'::::::::::::::::callAPI ENDED::::::::::::::::::');
 error_log('::::::::::::::::callAPI ENDED::::::::::::::::::');
...
/******************end using api http ********************/
		...
		catch (Exception $e)
		{
			...
			die("Something Went wrong With Your Request. Please Try Again Later !");
		}
	}
